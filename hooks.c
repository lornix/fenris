  ...
  loopover:
    ...
    switch (d.type) {
        ...
        case DMSG_STEP:
            ...
            // FIXME: 64bit
            sprintf(buf,
                    "At 0x%llx, advancing by %d local code instruction(s)...\n",
                    r.rip, modepar);
            // FIXME: 64bit
            if (INLIBC(r.rip))
                strcat(buf,
...
void break_exitcond(void)
{
    break_sendentity();
    send_asynctext
        (">> No more processes to trace. Fenris will now terminate.\n");
}
    ...
    for (i = 0; i < MAXBREAK; i++)
        if (bp[i].type == BP_ADDR)
            if (bp[i].param == addr) {
                ...
                // FIXME: 64bit
                sprintf(buf,
                        ">> LIBCALL breakpoint #%d (0x%x) stop at 0x%llx [%s].\n",
...
void break_syscall(unsigned int num)
{
    ...
    for (i = 0; i < MAXBREAK; i++)
        if (bp[i].type == BP_SYS)
            if (bp[i].param == num) {
                ...
                // FIXME: hardcoded?!? more than 300+ for i386
                sprintf(buf,
                        ">> SYSCALL breakpoint #%d [%d, %s] stop at 0x%llx [%s].\n",
                        i, num, scnames[num & 0xff], r.rip,
                        break_getname(r.rip));
                send_asynctext(buf);
            }
    ...
    if (curmode == MODE_SYSCALL) {
        ...
        sprintf(buf, ">> Syscall %d (%s) reached at 0x%llx [%s].\n", num,
                scnames[num & 0xff], r.rip, break_getname(r.rip));
        send_asynctext(buf);
    }
...
void break_nestdown(void)
{
    ...
    if (curmode == MODE_NEST) {
        if (current->nest < modepar) {
            ...
            // FIXME: 64bit
            sprintf(buf,
                    ">> Logical nesting stop point reached at 0x%llx [%s].\n",
                    r.rip, break_getname(r.rip));
            send_asynctext(buf);
        }
    }
    ...
    for (i = 0; i < MAXBREAK; i++)
        if (bp[i].type == BP_SIG)
            if (bp[i].param == signo) {
                ...
                // FIXME: 64bit
                sprintf(buf,
                        ">> Signal %d breakpoint #%d stop at 0x%llx [%s] (handler 0x%x).\n",
                        signo, i, r.rip, break_getname(r.rip),
                        get_handler(signo));
                send_asynctext(buf);
            }
    ...
    for (i = 0; i < MAXBREAK; i++)
        if (bp[i].type == BP_R) {
            ...
            if ((bp[i].param <= addr) && (bp[i].param2 >= addr)) {
                ...
                // FIXME: 64bit
                sprintf(buf,
                        ">> Read watchpoint #%d stop at 0x%llx [%s] (read of 0x%x).\n",
    ...
    for (i = 0; i < MAXBREAK; i++)
        if (bp[i].type == BP_W)
            if ((bp[i].param <= addr) && (bp[i].param2 >= addr)) {
                ...
                // FIXME: 64bit
                sprintf(buf,
                        ">> Write watchpoint #%d stop at 0x%llx [%s] (read of 0x%x).\n",
                        i, r.rip, break_getname(r.rip), addr);
                send_asynctext(buf);
            }
}
...
{
    send_synctext
        ("ERROR: I am running in 'no analysis' mode. This feature is not available.\n");
}
...
void break_tellwillresume(int i)
{
    ...
    if (i) {
        ...
        // FIXME: 64bitmode
        sprintf(buf,
                "WARNING: This syscall will resume upon continuation. If that is not what you\n"
                "want, type 'setreg rip 0x%llx' and set %%rax to a desired return value.\n",
                r.rip);
        send_asynctext(buf);
    } else
        send_asynctext
            ("This syscall will be not resumed. You might want to set %rax to a desired\n"
