...
void my_open(const char *name)
{
    ...
    int i;
    if (my_file)
        fatal("myfile already open");
    i = open(name, O_RDONLY);
    if (i < 0)
        PERROREXIT(name);
    if (fstat(i, &st))
        PERROREXIT("fstat on input");
    if (!(st.st_mode & S_IFREG))
        fatal("input is not a regular file");
    if (st.st_size <= 0)
        fatal("zero size input");
    my_file = mmap(0, st.st_size, PROT_READ, MAP_SHARED, i, 0);
    if (my_file == MAP_FAILED)
        PERROREXIT("mmap on input");
...
void get_buffers(void)
{
    ...
    while ((x = my_gets())) {
...

        {
            ...
            if (sscanf(x, "%d:", &miau) == 1) {
                if (forcepid != miau) {
                    if (!pidnotice)
                        STDERRMSG("[!] Only first process analyzed (%d but not %d or others).\n", forcepid, miau);
                    pidnotice = 1;
                    continue;
                }
            }
        ...
        if (!(cline % 941)) {
            STDERRMSG("[+] Collecting data... %0.02f%% (%d lines) done\r",
                      ((float)(my_ptr - my_file)) * 100.0 / ((float)(my_end - my_file)), cline);
        }
        ...
        if (!strncmp(x, "\\ new", 5)) {
            ...
            if (!strstr(x, "buffer candidate:"))
                continue;
            if (!(x = strchr(x, ':')))
                fatal("malformed \\ new line");
            x += 1;
            if (sscanf(x, " %x:%d", &addr, &len) != 2)
                fatal("malformed \\ new data");
            if (len < 0)
                fatal("\\ new length less than 0");
            if ((q = find_addr(addr)) >= 0) {
                if (!(b[q].fl & BFL_ASSUMED)) {
                    STDERRMSG("[!] line %d: new buffer %x:%d already exists as %x:%d.\n", cline, addr, len, b[q].addr,
                              b[q].size);
                    problems++;
                }
                continue;
            ...
            if (sscanf(x, "\\ discard: mem %x:%d", &addr, &len) != 2)
                fatal("malformed \\ discard");
            if (len < 0)
                fatal("\\ discard len less than 0");
            ...
            unsigned int a1, a2;
            x += 9;
            if (sscanf(x, "%x:%d -> %x:%d", &a1, &a2, &addr, &len) != 4)
                fatal("malformed \\ merge");
            ...
            if (T_verb)
                STDERRMSG("= Merged into #%d: %x | %x = %x:%d\n", q, a1, a2, addr, len);
            ...
            x += 14;
            if (sscanf(x, "%x:%d %x:%d (%*[^)]) -> %x:%d", &a1, &l1, &a2, &l2, &addr, &len) < 6)
                fatal("malformed \\ merge");
            while ((q = find_inrange(addr, addr + len)) >= 0) {
                if (T_verb)
                    STDERRMSG("- Discard: %x [#%d] in merge with %x:%d %x:%d\n", b[q].addr, q, a1, l1, a2, l2);
                b[q].st = ST_FREE;
                if (b[q].addr == a1)
                    any++;
                else if (b[q].addr == a2)
            ...
            b[btop].t = bufno;
            if (!any)
                STDERRMSG("[!] line %d: merge of non-existing buffers [%x | %x -> %x]\n", cline, a1, a2, addr);
            if (T_verb)
                STDERRMSG("= Merged into #%d: %x | %x = %x:%d\n", btop, a1, a2, addr, len);
            btop = -1;
            continue;
            ...
            if (!(x = strchr(x, '>')))
                fatal("malformed - remap line");
            x += 1;
            if (sscanf(x, " %x:%d", &addr, &len) < 2)
                fatal("malformed \\ new data");
            if (len < 0)
                fatal("- remap less than 0");
            b[btop].iaddr = b[btop].addr = addr;
            b[btop].isize = b[btop].size = len;
            b[btop].st = ST_CUR;
            b[btop].t = bufno;
            ...
            if (sscanf(x, "+ %x = %x:%d <off %d>", &q, &addr, &len, &q) != 4)
                continue;
            if (len < 0)
                fatal("assumed new length < 0");
            if ((q = find_inrange(addr, addr + len)) >= 0) {
                while (q >= 0) {
                    if ((b[q].addr == addr) && (b[q].size == (unsigned int)len))
                        goto getmeout;
                    if (T_verb)
                        STDERRMSG("= Adjust #%d into #%d: %x:%d -> %x:%d\n", q, btop, b[q].addr, b[q].size, addr, len);
                    b[q].st = ST_FREE;
                    q = find_inrange(addr, addr + len);
                }
            } else {
                if (T_verb)
                    STDERRMSG("+ Assumed new #%d: %x:%d\n", btop, addr, len);
            }
  ...
  bailout:
    ...
    if (!exitcond)
        STDERRMSG("[!] Warning: file truncated!\n");
...

    {
        ...
        for (i = 0; i < bufno; i++) {
            int j;
            if (!(i % 123))
                STDERRMSG("[+] Sorting buffers... %0.02f%% done\r", ((float)i * 100.0) / ((float)bufno));
            for (j = 0; j < breally; j++) {
    ...
    if (T_verb) {
        STDERRMSG("[+] BUFFERS: ");
        for (i = 0; i < breally; i++)
            STDERRMSG("%d [%d] %x:%d ", i, b[i].num, b[i].addr, b[i].size);
        STDERRMSG("\n");
    }
...
void parse_functions(void)
{
    ...
    strcpy(fname[0], "main");
    if (!(fnfile[0] = fopen(getname("F", 0), "w")))
        fatal("cannot open fnfile");
    ...
    while ((x = my_gets())) {
        ...
        STDERRMSG("[+] Found %d function calls, %0.02f%% done...\r", fnum,
                  ((float)(my_ptr - my_file)) * 100.0 / ((float)(my_end - my_file)));
      ...
      reanalyze:
        ...
        if (!strncmp(x, "...return", 9) || !strncmp(x, "...left", 7) || !strncmp(x, "...function", 11)) {
            ...
            if (nest - 1 >= 0)
                fprintf(fnfile[nest - 1], "<b><a href=\"#l%d\">%07d</a></b>  <i>%s</i>\n", cline, cline, ox);
          ...
          reprocess:
            ...
            // Add other objects! Buffer is not enough
                ...
                int add, f;
                if (sscanf(x, "* READ buffer %x", &add) != 1)
                    fatal("malformed read");
                f = find_addr(add);
                if (f < 0)
                    STDERRMSG("[!] line %d: READ on non-existing buffer %x.\n", cline, add);
                else {
            ...
            // Add other objects! Buffer is not enough
                ...
                int add, f;
                if (sscanf(x, "* READ local object %*s (%x)", &add) < 1)
                    fatal("malformed read");
                f = find_addr(add);
                if (f < 0)
                    STDERRMSG("[!] line %d: READ on non-existing buffer %x.\n", cline, add);
            ...
            // Add other objects! Buffer is not enough
                ...
                int add, f;
                if (sscanf(x, "* READ shared object %*s (%x)", &add) < 1)
                    fatal("malformed read");
                f = find_addr(add);
                if (f < 0)
                    STDERRMSG("[!] line %d: READ on non-existing buffer %x.\n", cline, add);
                ...
                int add, f;
                if (sscanf(x, "* WRITE buffer %x", &add) != 1)
                    fatal("malformed write");
                f = find_addr(add);
                if (f < 0)
                    STDERRMSG("[!] line %d: WRITE on non-existing buffer %x.\n", cline, add);
                else {
                ...
                int add, f;
                if (sscanf(x, "* WRITE local object %*s (%x)", &add) < 1)
                    fatal("malformed write");
                f = find_addr(add);
                if (f < 0)
                    STDERRMSG("[!] line %d: WRITE on non-existing buffer %x.\n", cline, add);
                ...
                int add, f;
                if (sscanf(x, "* WRITE shared object %*s (%x)", &add) < 1)
                    fatal("malformed write");
                f = find_addr(add);
                if (f < 0)
                    STDERRMSG("[!] line %d: WRITE on non-existing buffer %x.\n", cline, add);
                 ...
                 if (!b[findbynum(i)].yet) {
                    ...
                    switch (a[nest][i]) {
                        case 0:
                            ...
                            break;
                        case B_READ:
                            outf("<a href=\"#b%d\">r</a>", i);
                            break;
                        case B_WRITE:
                            outf("<a href=\"#b%d\">W</a>", i);
                            break;
                        case B_WRITE | B_READ:
                            outf("<a href=\"#b%d\">X</a>", i);
                            break;
                        default:
                            outf("?");
                    }
        ...
        if (!strncmp(x, "\\ discard", 9)) {
            ...
            int q;
            x += 14;
            if (sscanf(x, "%x:%d %x:%d (%*[^)]) -> %x:%d", &q, &q, &q, &q, &addr, &q) < 6)
                continue;
            if ((q = find_addr(addr)) >= 0) {
                ...
                STDERRMSG("[!] line %d: \\ merge on unknown %x.\n", cline, addr);
            continue;
        }
            ...
            if ((q = find_addr(addr)) >= 0) {
                ...
                STDERRMSG("[!] line %d: \\ new on unknown %x.\n", cline, addr);
            continue;
        }
            ...
            int q;
            unsigned int addr;
            if (sscanf(x, "\\ buffer %x modified", &addr) != 1)
                continue;
            if ((q = find_addr(addr)) >= 0)
                a[nest][b[q].num] |= B_WRITE;
            else
                STDERRMSG("[!] line %d: modified on unknown %x.\n", cline, addr);
            continue;
        }
        ...
        // move it to a separate function ;P
          ...
          handle_migration:
            ...
            *(x - 1) = 0;
            fprintf(fnfile[nest],
                    "<b><a href=\"#l%d\">%07d</a></b>  %s <font color=blue>==&gt; Click <a href=\"#f%d\">here</a> for trace of this libcall &lt;==</font>\n",
                    cline, cline, ox, fnum);
            nest++;
            if (nest >= MAXNEST)
                fatal("MAXNEST exceeded");
            ...
            *(x - 1) = 0;
            fprintf(fnfile[nest],
                    "<b><a href=\"#l%d\">%07d</a></b>  %s <font color=blue>==&gt; Click <a href=\"#f%d\">here</a> for trace of this function &lt;==</font>\n",
                    cline, cline, ox, fnum);
            nest++;
            if (nest >= MAXNEST)
                fatal("MAXNEST exceeded");
                ...
                if (x[0] == '+') {
                    ...
                    if (strstr(x, "buffer candidate:")) {
                      ...
                      knowncont:
                        ...
                        x += 14;
                        if (sscanf(x, "%x:%d %x:%d (%*[^)]) -> %x:%d", &q, &q, &q, &q, &addr, &len) < 6)
                            continue;
                        if ((q = find_addr(addr)) >= 0) {
                            mem[b[q].num] |= B_READ;
                            {
                        ...
                        if ((q = find_addr(addr)) >= 0)
                            ...
                            STDERRMSG("[!] line %d: \\ L discard on unknown %x.\n", cline, addr);

                        continue;
                    } else if (strstr(x, "\\ buffer")) {
                        x += 8;
                        if (!strstr(x, "modified"))
                            continue;
                        if (sscanf(x, "%x", &addr) < 1)
                            continue;
                        if ((q = find_addr(addr)) >= 0)
                            mem[b[q].num] = B_WRITE;
                        else
                            STDERRMSG("[!] line %d: \\ modified unknown %x.\n", cline, addr);
                        continue;
                    }
                } else
                    break;
                ...
                if (x[0] == '+') {
                    ...
                    if (strstr(x, "buffer candidate:")) {
                        ...
                        *strchr(strchr(tmp + 3, ' ') + 1, ' ') = 0;
                        ...
                        for (i = 0; i < nest + 1; i++)
                            outf("|&nbsp;");
                        outf("<a class=ttip href=\"#\" onMouseover=\"showtip(this,event,'%s')\" onMouseout=\"hidetip()\">" "<font color=magenta>%s</a>" NFI, ttip, strchr(tmp + 3, ' ') + 1);
                        migrated = 1;
                    ...
                    if ((!w) || (sscanf(w + 4, "%d", &i) != 1))
                        fatal("malformed @ line.");
                    if (strstr(x, "duplicate"))
                        modtable[i] = MOD_DUP;
                    else if (strstr(x, "created"))
                        modtable[i] = MOD_OPEN;
                    else if (strstr(x, "removed"))
                        modtable[i] = MOD_CLOSE;
                    else
