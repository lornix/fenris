...
int get_dword_sock(int sock)
{
    ...
    if (read(sock, &ret, 4) != 4)
        FATALEXIT("short read in get_dword_sock in Fenris");
    fcntl(sock, F_SETFL, O_NONBLOCK);
    return ret;
}
...
void *send_message(int mtype, void *data, void *store)
{
    ...
    switch (mtype) {
        ...
        case DMSG_NOMESSAGE:
            break;                     /* don't send, just check async */
        ...
        default:
            FATALEXIT("unknown message type in send_message");

    }

    if (mtype != DMSG_NOMESSAGE) {
        if (dlen && !data)
            FATALEXIT("message needs data but data is NULL");
  ...
  read_loop:
    ...
    if (x.magic1 != DMSG_MAGIC1)
        FATALEXIT("incorrect magic1 from Fenris");
    if (x.magic2 != DMSG_MAGIC2)
        FATALEXIT("incorrect magic2 from Fenris");
    ...
    if (x.type == DMSG_ASYNC) {
        ...
        if (x.type != DMSG_REPLY)
            FATALEXIT("invalid message type from Fenris");
        ...
        switch (mtype) {
            ...
            case DMSG_NOMESSAGE:
                ...
                FATALEXIT("no sync response expected for DMSG_NOMESSAGE");
                break;
            ...
            case DMSG_GETMEM:
                a = get_dword_sock(sd);
                if (a < 0 || a >= MAXFENT - 4)
                    FATALEXIT("excessive data length in DMSG_GETMEM");
                memcpy(store, &a, 4);
                b = 0;
                ...
                while (b < a) {
                    ...
                    if (inc <= 0)
                        FATALEXIT("short read on DMSG_GETMEM");
                    b += inc;
                }
                fcntl(sd, F_SETFL, O_NONBLOCK);
                break;
            ...
            default:
                FATALEXIT("implementation error in send_message");
  ...
  nothing_to_read:
    ...
    if ((mtype != DMSG_NOMESSAGE) && (!got_sync))
        goto read_loop;
    fcntl(sd, F_SETFL, O_SYNC);
    ...
    while (1) {
        ...
        if (strlen(i_command)) {
            ...
            *f = 0;
            ...
            if (got > 1)
                STDERRMSG("Ambiguous command. Matches: %s\n", matches);
            else if (got == 0)
                STDERRMSG("Command '%s' unrecognized. Try 'help' for help.\n", main_cmd);
            else {
                if (!stopped)
                    STDERRMSG("WARNING: the code is still running!\n");
                ...
                if (cmptr->handler) {
                    ...
                    STDERRMSG("This command is not yet implemented.\n");
            }
