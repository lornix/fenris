...
struct Wcode_info {
  RSTree tree;		/* stores dissassembled addresses
			 * key-> opcode addr
			 * val-> opcode size
			 */
...
int Wcode_move_by(int rows) {
  ...
  if (rows<0) {
	...
	rows++;
        ...
        prev_pc+=bytes;
	if (prev_pc>pc)
	  continue; /* overlaped insn, continue main loop */
	/* check distance */
	  ...
	  if (prev_pc != pc)
	    ...
	    nd=Wcode_info.top_node; /* match succesed, continue main loop */
	    continue;
	  }
...
static int lock=0;	// negative - disable scrolling up
			// positive - disable scolling down
char * mem;
...
getline_optns.repeat_last=1;
	...
	z=0;
		...
		matches[0]=0;
			...
			if (nmatches>1){
			debug("Ambigious command. Matches: %s\n",matches);
   	  	        do_prompt(0);
			continue;
			}
