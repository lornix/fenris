...
struct fenris_mem {
    ...
    int owner;           // Owner ID (fnaddr[] offset), 0 - main
...
#define debug(x...)     do { snprintf(verybigbuf,sizeof(verybigbuf)-1,x); \
    fprintf(ostream,"%s",verybigbuf);  \
    if (T_dostep) break_append(verybigbuf); \
} while (0)
